...
### 해결 방식 — 5-layer 메모리 아키텍처

1. **자동 주입 (Layer 1)** — SessionStart 훅에서 최근 N개 세션을 Luna low로 요약해 새 세션에 자동 주입
2. **자연어 검색 (Layer 2)** — FTS5 + Arctic-ko 임베딩 hybrid (RRF) 로 과거 모든 세션·메모리 검색 가능. 세션 후보는 Luna 한 번으로 재순위·요약. `/recall` 스킬로 호출
3. **Memory Compiler (Layer 3)** — 세션 종료 시 Luna low가 결정/노하우/사실을 추출 → `memory/_procedural/_staged/` 임시 저장 → 사용자의 `/memory_review` 승인 후 영구 메모리 진입 (Sprint 13~14)
...
5. **모순 감지 (Layer 5)** — 신규 메모리와 top-5 기존 후보를 Luna 한 번으로 4-way 분류해 검토 큐에 추가
...
- **검출**: hybrid recall (FTS5 + Arctic-ko-MLX RRF) top-5 → Luna low 한 번의 batch 호출로 4-way 분류 (`metric_update` / `decision_reversal` / `fact_correction` / `no_conflict`)
...
- **Claude Code·Codex에서 공유 작동** — 별도 생성형 로컬 서버 없음
- **사용자 행동 제로 전제** — 수동으로 문서 만들지 않아도 자동 수집
- **로컬 검색, Luna 비동기 판단** — FTS5 + Arctic-ko (임베딩) `localhost:8081`; 생성형 LLM은 Codex CLI의 Luna low
- **폴백보다 명시적 skip** — Codex 실패·오프라인·한도 초과 시 해당 비동기 작업만 건너뛰며 로컬 LLM으로 우회하지 않음
...
- Codex CLI + `gpt-5.6-luna` low (추출·요약·별칭·검색 보조·모순 감지)
